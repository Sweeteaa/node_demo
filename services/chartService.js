...
//回收订单指定日期各种类所拥有的订单数 http://127.0.0.1:3001/chart/recycleday/:type
exports.getRDay = ((req, res)=>{
    db.query(`select count(1) from nv_users_orders where addTime = '${req.params.addTime}' and type='${req.params.type}'`, (err, results)=>{
        // SQL 语句执行失败
        if (err) return res.send({status:1, message:err})
...
//回收订单指定日期各种类所拥有的订单数 http://127.0.0.1:3001/chart/useday/:type
exports.getUDay = ((req, res)=>{
    db.query(`select count(1) from nv_users_useorders where addTime = '${req.params.addTime}' and type='${req.params.type}'`, (err, results)=>{
        // SQL 语句执行失败
        if (err) return res.send({status:1, message:err})
